(()=>{
  ...
  window.renderArtifacts=function(ev){
    ...
    for(const e of Array.isArray(ev)?ev:[]){
      ...
      for(const x of evidence){const kind=String(x?.kind||''),ref=String(x?.ref||'');if(kind==='github-pr')addLink(ref,'🔧 変更内容を見る','GitHub');else if(kind==='preview'||kind==='artifact')addLink(ref,'🌐 確認用ページを開く','記事プレビュー');else if(kind==='published-url'||kind==='public-url')addLink(ref,'🌐 公開ページを見る','公開ページ');else if(kind==='image')addLink(ref,'🖼️ 画像を確認','画像')}
      if(['preview-ready','publish-complete','published','pr-created'].includes(stage)||['preview','published','publication','artifact'].includes(type)){for(const url of [...collectUrls(e)]){try{const u=new URL(url),p=u.pathname.toLowerCase();if(u.hostname.includes('netlify'))addLink(url,'🌐 確認用ページを開く','記事プレビュー');else if(u.hostname==='github.com'&&p.includes('/pull/'))addLink(url,'🔧 変更内容を見る','GitHub');else if(/\.(webp|png|jpe?g)$/.test(p))addLink(url,'🖼️ 画像を確認','画像');else if(u.hostname.includes('denkicontrol.com'))addLink(url,'🌐 公開ページを見る','公開ページ')}catch(_){}}}
    }
  ...
  if(badge){deleteButton=document.createElement('button');deleteButton.id='deleteCase';deleteButton.className='deleteCase';deleteButton.type='button';deleteButton.textContent='案件キューを削除';deleteButton.hidden=true;badge.insertAdjacentElement('afterend',deleteButton);deleteButton.addEventListener('click',async()=>{const commandId=commandIdFromJob(selectedJob);if(!commandId)return;if(!confirm('この案件キューを削除しますか？\n画面上の進行案件から外します。監査用の履歴データ自体は削除しません。'))return;deleteButton.disabled=true;deleteButton.textContent='削除中…';try{const{error}=await sb.rpc('ai_editorial_close_command',{p_command_id:commandId});if(error)throw error;selectedJob=null;manualSelection=false;try{const box=document.querySelector('#events');if(box){box.dataset.job='__home__'}}catch(_){};await refresh();selectedJob=null;manualSelection=false;if(typeof window.render==='function')window.render();requestAnimationFrame(()=>{selectedJob=null;manualSelection=false;try{if(typeof window.render==='function')window.render()}catch(_){}})}catch(err){console.error('[AI編集部] delete queue failed',err);alert('案件キューを削除できませんでした。もう一度お試しください。')}finally{deleteButton.disabled=false;deleteButton.textContent='案件キューを削除';syncDeleteButton()}})}
  function syncDeleteButton(){if(!deleteButton)return;const box=document.querySelector('#events'),id=commandIdFromJob(selectedJob);deleteButton.hidden=!id||box?.dataset?.job==='__home__'}
  ...
  function syncComposerLock(){const active=activeCommand(),locked=Boolean(active)||sending;send.disabled=locked;document.querySelectorAll('.weeklyButton').forEach(b=>{if(!b.dataset.unlockedTitle)b.dataset.unlockedTitle=b.title||'';b.disabled=locked||b.dataset.weekCompleted==='1';b.title=active?'現在の案件が完了または終了するまで新しい曜日案件は開始できません。':b.dataset.unlockedTitle});if(active){const state=String(active.last?.state||'進行中').toUpperCase(),title=String(active.first?.summary||'現在の案件').replace(/^提案[:：]\s*/,'').slice(0,60);lock.hidden=false;lock.textContent=`🔒 現在の案件が${state}です。「${title}」が完了または管理者判断で終了するまで新しい案件は登録できません。`;if(!sending)send.textContent='処理中'}else if(sending){lock.hidden=false;lock.textContent='案件を登録しています。重複登録を防ぐため一時的に操作をロックしています。'}else{lock.hidden=true;send.textContent='登録'}syncDeleteButton()}
  input.addEventListener('input',()=>{count.textContent=input.value.length+' / 4000';resize()});input.addEventListener('keydown',e=>{if((e.ctrlKey||e.metaKey)&&e.key==='Enter')form.requestSubmit()});
  form.addEventListener('submit',async e=>{e.preventDefault();if(activeCommand()){syncComposerLock();setMeta('現在の案件が完了または終了してから次の案件を登録してください。','error');return}const instruction=input.value.trim();if(!instruction){setMeta('指示を入力してください。','error');return}const {data:{session},error:sessionError}=await sb.auth.getSession();if(sessionError||!session){setMeta('ログイン状態を確認できません。','error');return}sending=true;send.textContent='登録中…';syncComposerLock();setMeta('案件キューへ登録しています…');try{const r=await fetch(COMMAND_ENDPOINT,{method:'POST',headers:{Authorization:'Bearer '+session.access_token,apikey:SUPABASE_KEY,'Content-Type':'application/json'},body:JSON.stringify({instruction,requested_count:requestedCount(instruction),options:{source:'dashboard-v0.7.3',execution_model:'chatgpt-editor-in-chief',auto_process:false}})});const raw=await r.text();let d={};try{d=raw?JSON.parse(raw):{}}catch{d={raw}};if(r.status===401){lockApp('ログインの有効期限が切れました。もう一度ログインしてください。');return}if(r.status===403)throw new Error('このアカウントには指示権限がありません。');if(!r.ok)throw new Error(d?.message||d?.detail?.message||d?.detail?.detail?.message||d?.error||'指示の登録に失敗しました。');input.value='';resize();count.textContent='0 / 4000';setMeta('案件を登録しました。完了または終了するまで新規登録はロックされます。','ok');manualSelection=false;await refresh()}catch(err){console.error('[AI編集部] command submit failed',err);setMeta(err instanceof Error?err.message:'指示の登録に失敗しました。','error')}finally{sending=false;syncComposerLock()}});
