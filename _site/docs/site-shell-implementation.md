...
本書の初期実装対象は記事のヘッダー・フッター。本文・記事内CSS・記事内レイアウト・新着・人気一覧・記事データ管理は変更しない。後続で採用した日英トップ・相談・キャリア入口の4ページは [追加枠の実装](additional-page-shells.md) に従い、同じヘッダー正本と専用構造を保持する追加フッター枠を使用する。
...
管理者の明示依頼により、`.github/site-shells/components/affiliate-disclosure.njk` を文面と配置の単一正本として、shell.njkが登録済み全37フッターvariantの末尾へ1段落を追加する。日本語は「当サイトはアフィリエイト広告を利用しています。」、英語は同趣旨の文。追加4ページの生成器も同じ正本を使用する。既存本文の個別広告注記・リンクは残す。記事HTMLはbuild:shellsで再生成し、今回の1段落以外の全バイト一致を確認して関連ハッシュを整合する。表示変更のため従来の共通化比較の差分0基準とは区別する。具体的な公開状態は移行記録を参照。
