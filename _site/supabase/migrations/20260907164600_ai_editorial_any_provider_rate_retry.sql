...
begin
  ...
  v_match := regexp_match(v_row.last_error, 'try again in\s+(?:(\d+)h)?(?:(\d+)m)?(?:(\d+(?:\.\d+)?)s)?', 'i');
  ...
  if now() < v_retry_at then
    return jsonb_build_object(
      ...
      'command_id',v_row.id,
      'retry_at',v_retry_at,
      'seconds_remaining',greatest(0,extract(epoch from (v_retry_at-now()))::integer)
    );
  end if;
  ...
  return jsonb_build_object(
    ...
    'command_id',v_row.id,
    'retry_at',v_retry_at,
    'provider',case when v_row.last_error ~* 'gemini|generativelanguage|resource_exhausted|quota exceeded' then 'gemini' else 'openai' end
  );
end;
$function$;
