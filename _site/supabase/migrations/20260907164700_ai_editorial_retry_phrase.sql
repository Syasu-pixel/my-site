...
begin
  ...
  where status='failed' and last_error is not null
    and last_error ~* '(rate[ -]?limit|rate_limit_exceeded|openai\s+429|gemini\s+429|quota exceeded|resource_exhausted|requests per day|tokens per minute)'
  ...
  if v_hours=0 and v_mins=0 and v_secs=0 then v_mins := 5; end if;
  v_retry_at := v_row.updated_at + make_interval(hours=>v_hours, mins=>v_mins, secs=>v_secs);
  if now() < v_retry_at then
    return jsonb_build_object('claimed',false,'reason','cooldown','command_id',v_row.id,'retry_at',v_retry_at,'seconds_remaining',greatest(0,extract(epoch from (v_retry_at-now()))::integer));
  end if;
  ...
  return jsonb_build_object('claimed',true,'command_id',v_row.id,'retry_at',v_retry_at,'provider',case when v_row.last_error ~* 'gemini|generativelanguage|resource_exhausted|quota exceeded' then 'gemini' else 'openai' end);
end;
$function$;
