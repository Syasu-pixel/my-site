(()=>{
  ...
  if(badge){deleteButton=document.createElement('button');deleteButton.id='deleteCase';deleteButton.className='deleteCase';deleteButton.type='button';deleteButton.textContent='案件キューを削除';deleteButton.hidden=true;badge.insertAdjacentElement('afterend',deleteButton);deleteButton.addEventListener('click',async()=>{const commandId=commandIdFromJob(selectedJob);if(!commandId)return;if(!confirm('この案件キューを削除しますか？\n画面上の進行案件から外します。監査用の履歴データ自体は削除しません。'))return;deleteButton.disabled=true;deleteButton.textContent='削除中…';try{const{error}=await sb.rpc('ai_editorial_close_command',{p_command_id:commandId});if(error)throw error;manualSelection=false;await refresh()}catch(err){console.error('[AI編集部] delete queue failed',err);alert('案件キューを削除できませんでした。もう一度お試しください。')}finally{deleteButton.disabled=false;deleteButton.textContent='案件キューを削除';syncDeleteButton()}})}
