(()=>{
  ...
  function previewReady(){return eventsForSelected().some(ev=>{const d=ev&&ev.discussion&&typeof ev.discussion==='object'?ev.discussion:{};const stage=String(d.stage||ev?.event_type||'').toLowerCase();const verifiedBy=String(d.preview_verified_by||'').toLowerCase();return stage==='preview-ready'||d.preview_verified===true||verifiedBy.includes('preview-probe')||verifiedBy.includes('http-preview-probe')})}
