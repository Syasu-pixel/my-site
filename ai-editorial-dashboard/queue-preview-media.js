(()=>{
  ...
  style.textContent=`
    ...
    .queuePreviewFrame{width:100%;height:420px;border:1px solid #dce3ee;border-radius:13px;background:#f7f9fc}.queuePreviewWaiting{min-height:180px;display:flex;align-items:center;justify-content:center;text-align:center;border:1px dashed #cbd6e4;border-radius:13px;background:#f8faff;color:#596579;font-size:12px;font-weight:800;line-height:1.8;padding:24px}.queuePreviewMedia{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:9px;margin-top:10px}.queuePreviewMediaCard{min-width:0;border:1px solid #dce3ee;border-radius:12px;padding:7px;background:#f9fbff}.queuePreviewMediaCard.ogp{grid-column:span 2}.queuePreviewMediaCard img{display:block;width:100%;aspect-ratio:16/9;object-fit:cover;border-radius:8px;background:#edf1f6}.queuePreviewMediaCard.ogp img{aspect-ratio:1.91/1}.queuePreviewMediaLabel{display:flex;justify-content:space-between;gap:6px;margin-top:6px;font-size:9px;font-weight:800;color:#526176}.queuePreviewEmpty{font-size:10px;color:#7b879b;margin-top:8px}
    ...
    @media(max-width:760px){.queuePreviewPanel{margin:14px 0 2px;padding:10px}.queuePreviewFrame{height:320px}.queuePreviewWaiting{min-height:150px;font-size:11px;padding:18px}.queuePreviewMedia{grid-template-columns:1fr 1fr}.queuePreviewMediaCard.ogp{grid-column:1/-1}.queueMediaEvent .queueMediaBubble{width:92vw}.queueMediaThumb{max-height:230px}}
  `;
  ...
  function renderEnhancements(){
    ...
    const items=mediaItems();const sig=JSON.stringify([id,previewUrl(),previewReady(),items.map(x=>[x.url,x.createdAt])]);
