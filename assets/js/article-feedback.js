(() => {
  ...
  const lockVote = (vote, message = '投票済みです。ご協力ありがとうございます。') => {
    locked = true;
  ...
  const submitVote = async (vote) => {
    ...
    try {
      ...
      if (response.status === 409 && payload?.error === 'already_voted') {
        const existingVote = payload.vote === 'helpful' || payload.vote === 'not_helpful' ? payload.vote : vote;
        lockVote(existingVote);
        return;
      }
      ...
      lockVote(vote, '回答ありがとうございます。投票済みです。');
    } catch (error) {
      console.error('[article feedback]', error);
      status.textContent = '送信できませんでした。もう一度お試しください。';
    } finally {
      if (!locked) buttons.forEach((button) => { button.disabled = false; });
    }
  };
