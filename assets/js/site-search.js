...
(() => {
  ...
  style.textContent = `
    .article-feedback-card{text-align:center;background:linear-gradient(180deg,#f8fbff,#fff)}
  ...
  const getVoterKey = () => {
    let key = localStorage.getItem(voterKeyStorage);
    if (!key) {
      key = makeVoterKey();
      localStorage.setItem(voterKeyStorage, key);
    }
    return key;
  };
  ...
  const previousVote = localStorage.getItem(voteStorage);
  ...
  const submitVote = async (vote) => {
    ...
    try {
      ...
      localStorage.setItem(voteStorage, vote);
  ...
  if (previousVote === 'helpful' || previousVote === 'not_helpful') renderSelection(previousVote);
})();