...
- 新しいチャット、引き継ぎ、シリーズ続編、設備設計等の継続案件では、共通ルールに加えて案件固有の正本・decision log・I/O表・仕様書を確認する。
- 案件資料の探索はGitHub `main` だけに限定しない。関連する未マージPR・Draft PR・作業ブランチも確認し、`main` に無いことだけで「元データなし」と判断しない。
- 既存の案件正本がある場合、同目的の新しい `master-spec` を別場所へ作る前に、既存正本の状態・採用範囲・更新履歴と統合可否を確認する。
- 未マージPR/ブランチ上の案件資料は、採用済み共通ルールではない。しかし案件継続の一次資料として内容を確認し、main上の公開済み記事・後続PR・管理者判断との整合を取る。
- 共通ルールと案件固有仕様が矛盾する場合、安全・法令・公開権限等の上位原則を優先し、案件固有の仕様差は勝手に消さず、矛盾を明示して整理する。
...
## 8. HTML構造
- 管理者が共通ひな形方針を採用した後の適用対象・初回移行・新規記事は、[site-template-policy.md](site-template-policy.md) 第1・6節の限定例外に従い、以下の完全コピー指定を構造分離方式へ読み替える。表示・内容保存と公開承認の要件は緩和しない。
