...
現在の表示と内容を変えず、記事ヒーローの枠・固有情報・専用CSSを分離する。初回はmanifestに明示した8記事のみ。日本語通常/長タイトル、工具、CTAなし2件、広告表記、英語通常/長タイトルを含める。トップ、相談サービス、カテゴリ、キャリア入口、設計シリーズは対象外。本文全体、日付、SEO、URL、画像制作、記事改善へ拡張しない。
...
初回のCSS分離は字句単位で保守的に行う。空白・順序まで同じルールだけを同一プロフィールへまとめるため、8記事で160ファイルになる。160の別デザインを推奨するものではない。短いルール単位のため、プロフィールのファイル名は初期内容のハッシュを識別子とする。ページ/元CSS位置→プロフィールの対応を経由して編集する。
...
既存の [build-integrated-review.mjs](../scripts/build-integrated-review.mjs) 内で、最新記事からhero-pages.jsonを生成する。既存のヘッダー/目次/関連記事/評価の生成後に、登録記事だけへhero.njkとCSSを適用する。candidateとreviewは従来の境界を維持する。Pagesが呼ぶ同じ生成入口に接続しており、生成HTMLを手直しする別工程はない。
...
初回は8件のヒーロー/CSSを含む再構成HTML全バイト一致、統合後の断片一致、全288記事の既存保全検査を行う。単体検査は共通枠を一度変更して8件に伝播すること、共有CSSは利用ページにだけ伝播すること、対象外不変、未知構造/古いCSS/古い抽出データ/重複対象の拒否を別コピーで確認する。

画面は8記事×4幅の32組を変更前後で比較し、タイトル・要点・広告・CTAの位置、背景画像の濃度/位置/サイズ、ヘッダーとの関係、折返しを確認する。比較結果は実行したcommitと成果物側へ記録し、コードの存在だけで合格とはしない。実機Safari、スクリーンリーダー、文字拡大は別途必要。

全記事への展開時は最新mainと他担当変更を取り込み、対象台帳を増やす前に未対応構造を検出する。構造差や用途差を欠落扱いして変換しない。専用ページ群は別variantの設計と比較が必要。本文改稿、CTA追加、日付追加、検索需要に基づく再設計を混ぜない。
