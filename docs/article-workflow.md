...
### PR確認時の必須チェック追加（英語記事 talk-thread）
- 英語記事に `talk-thread` がある場合、先輩・後輩画像が `guide-characters` を参照しているか確認する
- `talk-avatar img` の参照先が存在するか確認する
- 公開ページで alt テキストだけ表示されていないか確認する
