...
## 活動範囲の確定ルール（2026-09-08追記）

ページ全体が二人の活動場所。中央の専用部屋や作業スペースへ閉じ込めない。実際のチャット吹き出し、カード、ボタンの縁を足場にして移動・着座・登攀する。足場はDOMの現在位置から取得し、スクロール・リサイズ・案件切替に追従する。左の案件キュー下に十分な空白がある場合はお茶休憩の場所とし、家具を持ち込む。空白がない場合は休憩家具を出さず、本文や操作欄へ重ねない。キャラのレイヤーはクリックを通す。中央チャットを隠したり高さを消費したりしない。

今回の試作更新：全画面オーバーレイ、DOM足場候補への座標移動、左空白の検出、休憩場所への移動と簡易テーブル表示。歩行の脚運び、手足での登攀、家具の搬入・給湯動作はまだ未実装。座標移動をそれらの完成と扱わない。

## 重力・接地・奥行き（2026-09-08確定）

通常移動の地面は表示ページの最下端。空中の二点間を斜めに補間して移動させない。上に行く場合は、梯子を搬出→運搬→設置→手足で登る→足場へ乗り移る。帰路も梯子を降り、片付ける。足場が消える・スクロールする場合の復帰を実装してから上方移動を有効化する。

キャラ周囲はアルファ透過素材を使い、白い服やヘルメットを白背景と一緒に消さない。白を全て抜く処理や乗算合成で代用しない。足元に影、梯子は体の後ろ、天板は体の手前とし、物理的な接点と前後関係を統一する。

今回反映：下端への接地固定、空中移動停止、接地影、テーブルの下端配置と二人の到着後表示、家具と人物の描画順。未反映：透過素材への交換、脚運び、梯子の搬入・登降、着座・給湯。これらを実装済みと案内しない。
