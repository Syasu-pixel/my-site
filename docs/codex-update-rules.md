...
- `en/index.html` の New articles / Latest articles / Recent articles 相当の右カラム・新着リストは、原則10件表示に固定する。
- `index.html` の新着記事・右カラム系リストも原則10件表示に固定する。
- 11件目以降をそのまま追加して、トップページを縦に間延びさせない。
- 新着リストに新記事を追加する場合は、最新10件になるよう古い記事を押し出す。
- ただし既存の個別設計がある場合は、その設計を壊さず、表示件数を増やし続けない方針を優先する。
- Step 2導線追加時は、トップ棚4件固定・新着10件固定を確認してから `safe to close` を出す。
