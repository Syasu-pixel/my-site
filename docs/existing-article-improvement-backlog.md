...
このファイルは、Google Search Console の検索流入上位記事を中心に、既存記事を1本ずつ改善して完成・凍結していく進捗を管理するためのバックログです。

## 運用ルール

- 元の優先順は `docs/search-top-content-audit-2026-09-10.md` を基準にする。
...
- [ ] `articles/plc-io-unit-basic.html` — PLC I/Oユニット — 作業中（PR #1458 / 本文見直し / 関連記事画像カード化 / 追加画像は保留）
- [ ] `articles/plc-xymd-device-basic.html` — PLC X/Y/M/Dデバイス — 作業中（PR #1459 / 本文見直し / 関連記事画像カード化 / 追加画像は保留）
- [ ] `articles/one-shot-circuit-basic.html` — ワンショット回路 — 作業中（本文見直し / 関連記事画像カード化 / 追加画像は保留）
- [ ] `articles/terminal-block-basic.html` — 端子台 — 作業中（本文見直し / 関連記事画像カード化 / 安全文言整理 / 追加画像は保留）
