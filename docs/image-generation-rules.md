...
### 0.5) 役割分担の補足（運用）
- 記事画像生成は ChatGPT が担当する。
- Codex は画像生成を担当しない。
- Codex が画像ファイルを扱う場合は、採用済み画像を指定パスへ配置し、存在確認するだけにする。
- Codex は画像を切り分けたり、コラージュ化したり、別用途画像をまとめたりしない。

### 1) 記事画像ジェネレーターの基本原則（全記事共通）
- 記事画像は、ChatGPTでこれまで作ってきた教育系・図解系の雰囲気を維持する。
- 基本トーンは、白背景ベース・青系中心・整理された図解・スマホでも理解しやすい見た目にする。
- 画像は原則 `.png` で作成する。
- 1回の画像生成では1枚の用途だけを作る。
- 1枚に複数用途を混ぜない。
- `hero / ogp / overview / comparison / flow / check-flow` などの役割を混ぜない。
...
- 参照先：
  - 正本: `assets/images/character-templates/senpai-kouhai-character-template.png`
  - 補助参照（旧テンプレート）: `assets/images/character-templates/senpai-kouhai-chibi-character-template.png`
- 人物キャラクターを含む画像では、必ず正本テンプレートを参照する。
- 参照なしで人物キャラクターを生成しない。
- 新しい別キャラを勝手に作らない。
...
- hero画像
  ...
  - hero画像はその記事セットのアンカー画像になるため、1枚目でテーマ逸脱が起きた場合は絶対に採用しない。
  - hero画像が広告LP風、WebサイトUI風、写真素材風、別ジャンルになった場合は、その後続画像へ進まない。
  - hero採用前に、記事テーマ・キャラ・機器・用途・左余白の5点を必ず確認する。
  - heroがNGの場合、同じ方向で修正せず、プロンプトをより制御盤・電気制御寄りにして完全再生成する。
- OGP画像
...
### 公式用語ベースのラダー・命令語画像ルール
- ラダー図・命令語・GOT画面・PLCデバイスを含む画像では、記事ごとの公式参照メモまたは公式資料で確認した用語だけを使う。
- ChatGPT は画像生成前に、画像内で使用してよい用語・デバイス名・命令語を整理する。
- 画像内に未確認の命令語、未確認のデバイス名、メーカー固有の画面名、型式固有のパラメータを入れない。
- 「それっぽいGX Works3画面」「実在ソフトのスクリーンショット風」「三菱電機公式画面風」はNGとする。
- 画像は公式表記を参考にした概念図・模式図として作る。
- 英語記事の画像内テキストは、英語公式資料で確認した用語を優先する。
- 日本語記事の画像内テキストは、日本語公式資料で確認した用語を優先する。
- 日本語元記事の英語化画像では、日本語直訳ではなく、英語公式資料または既存英語記事で採用した用語を使う。
- 画像内文字は最小限にし、1枚の画像に多数のデバイス名・命令語・パラメータを詰め込まない。
- 公式資料で確認できない細かい仕様は画像化しない。
- 読めない小さい文字、崩れた英数字、存在しない命令語、架空のデバイス名が出た画像は不採用とする。
...
### 5.85) キャラクター固定の運用メモ（再確認）
- キャラクター固定条件の正本は「2) キャラクター統一ルール（全記事共通）」を参照する。
- 生成時は、キャラクターを図解補助にとどめ、図解や文字より目立たせない。
- 先輩・後輩ともに、ヘルメットに文字ラベルを入れない。

### 5.86) 画像内テキスト固定ルール
- 画像内テキストは、英語記事では英語のみ、日本語記事では日本語のみを原則とする。
- 英語記事用画像に日本語を混ぜない。
- 日本語記事用画像に不要な英語ラベルを混ぜすぎない。
- 画像内テキストは短く、大きく、読みやすくする。
- 長文説明を画像内に入れない。
- 小さすぎる注釈を大量に入れない。
- スペルミスが出やすい長い文章は避ける。
- 複雑な説明はHTML本文側に任せ、画像内は短い見出し・ラベル・STEP名に限定する。
- 画像内にファイル名、保存名、記事slugを表示しない。
- 画像内に `1枚目` / `2枚目` / `No.1` などの生成管理ラベルを入れない。
...
- 実在PLCソフト画面風
- 実在ロゴ入り
- 記事テーマと関係ない風景、旅行、桜、街並み、Web制作風デザイン
...
- 保存名が1つだけ明記されているか
- 用途が1つだけに限定されているか
- hero / OGP / overview / flow / ladder-logic / field-checks のどれかが明確か
- その用途以外の内容を混ぜていないか
- キャラクターテンプレートを確認したか
- hero採用後の後続画像なら、採用済みheroの雰囲気も参照しているか
- 画像内テキストは短くできているか
- 日本語/英語が混在していないか
- コラージュや6分割にならない指示になっているか
- 記事slugと画像用途をプロンプト冒頭に明記したか
- 「この画像は電気制御・制御盤・PLC・現場実務の記事画像である」と明記したか
- 無関係ジャンルを禁止語として明記したか
- WebサイトUI、LP、広告、写真風、一般サービス画面を禁止したか
- control-panel系記事では、制御盤・電気部品・配線・盤内機器のいずれかが主役になるよう指定したか
- 画像内にCTAボタン、ナビゲーション、ログイン、料金プラン、FAQなどWebサイト要素を入れない指定をしたか

### 5.89) 生成後チェックルール
画像生成後に、採用判定前に以下を確認する。

- 保存名の用途に合っているか
- 1枚1用途になっているか
- コラージュや複数パネルになっていないか
- WebページUI風になっていないか
- hero画像なら左側余白が確保されているか
- OGP画像なら中央タイトル中心で情報量が少ないか
- 記事内画像なら図解・フロー・チェック内容が主役になっているか
- キャラクターがテンプレートから大きく外れていないか
- ヘルメットに文字が入っていないか
- 先輩と後輩の役割が入れ替わっていないか
- 画像内テキストが読めるか
- スペルミスや崩れた文字がないか
- 日本語/英語が混ざっていないか
- ファイル名ラベルや番号ラベルが入っていないか
- NG項目がある場合は、その画像だけ再生成する
- 記事slugと直接関係する画像になっているか
- 無関係なWebサイト・広告・LP風になっていないか
- 電気制御の記事なのに、フィットネス・美容・一般サービス・人物写真・生活シーンになっていないか
- control-panel系記事では、制御盤または盤内機器が主役になっているか
- Webサイトナビゲーション、ログインボタン、CTAボタン、料金表、FAQなどが入っていないか
- 生成結果が記事テーマから外れた場合は「惜しい」と判断せず即NGにする
- GX Works3命令語シリーズでは、ラダー風表現が実在ソフトUIの再現になっていないか
- GX Works3命令語シリーズでは、命令ブロックが抽象図として見えるか
- 英語記事用画像なら英語のみ、日本語記事用画像なら日本語中心になっているか
- 外部ブランド名や別サイト名が入っていないか
- キャラクターを使う場合、character template と雰囲気が合っているか
- 色味が白背景・青系中心で、サイト全体のトーンに合っているか
- 1枚に詰め込みすぎていないか
- 情報が多い場合、overview / comparison / check-flow へ分割できているか

### 5.90) 画像生成時の運用メモ
- ユーザーが「全部ください」と言った場合でも、内部的には保存名順に1枚ずつ処理する。
- まとめ画像を作るのではなく、個別画像を連続で出す。
- ユーザーが「採用」と言った画像だけを、その保存名の画像として扱う。
- 不採用画像は保存名一覧に含めない。
- 採用済み画像の保存名を後から曖昧にしない。
- 最後に必ず、採用済み画像と保存名の対応一覧を出す。


### 5.91) 記事テーマ逸脱・広告LP暴走の禁止ルール
- 画像生成では、記事slug・記事タイトル・記事テーマに直接関係する要素だけを描く。
- 電気制御・PLC・制御盤・電気工事の記事画像で、以下のような無関係ジャンルを出してはいけない。
  - フィットネス
  - 美容
  - 健康食品
  - オンライン講座広告
  ...
  - スマホアプリUI
  - 広告バナー
- `control-panel-*` の記事では、最低限以下のどれかを必ず画像内の主役として入れる。
  - control panel
  - electrical cabinet
  - terminal block
  - DIN rail
  ...
  - grounding bar
  - label / nameplate
- 記事テーマと無関係な人物写真・生活シーン・Webページ画面が出た場合は即NG。
- 画像が「記事用図解」ではなく「Webサイトのスクリーンショット」「LPデザイン」「広告デザイン」に見える場合は即NG。
- 生成された画像に記事タイトルと無関係なサービス名、ブランド名、UIナビゲーション、ログインボタン、CTAボタンなどが出た場合は即NG。
- 画像内に日本語が混ざった英語記事用画像も即NG。
- 記事テーマから外れた1枚目が出た場合、その画像を切り分け・修正・流用せず、完全に破棄して再生成する。
...
用途:
{hero / ogp / overview / flow / field-checks}

記事カテゴリ:
電気制御 / PLC / 制御盤 / 電気工事 / 現場実務

この記事のテーマ:
{記事テーマを1文で書く}

この画像で必ず描く主役:
{制御盤、盤内コンセント、ブレーカー、端子台など}
...
参照画像:
- `assets/images/character-templates/senpai-kouhai-character-template.png`（画像生成の正本）
- 必要に応じて `assets/images/character-templates/senpai-kouhai-chibi-character-template.png`（補助参照）
...
#### GX Works3命令語シリーズの画像表現ルール
GX Works3命令語シリーズの記事画像では、以下のような「自作の概念図・模式図」は使用してよい。

使用してよい表現：
- 簡略化したラダー風の図
- 実在ソフト画面ではない、抽象化された命令ブロック
- `ADD` / `SUB` / `MOV` などの命令名を使った模式図
- `Source value` / `Destination device` / `Result` のような英語ラベル
- `D10 + K1 → D10` のような初心者向けの抽象例
- `Source values → ADD/SUB → Result` のような演算フロー
- データレジスタを表す抽象ブロック
- 必要に応じた、簡略化されたラダー接点・命令ブロック・出力先の流れ
- 公式画面の再現ではなく、記事用に描き直した教育用図解
- バイナリや命令コード風の情報を使う場合は、公式画面風ではなく、記事内容を補助する自作の抽象図として扱う

ただし、以下は禁止：
- 実在のGX Works3画面のスクリーンショット風
- 三菱電機公式画面の再現
- 実在ロゴ
- 実在ソフトUIのウィンドウ、メニュー、ツールバー、プロジェクトツリー再現
- 公式資料からのコピーに見える表・画面
- 型式固有の未確認仕様
- 未確認のデバイス範囲
- 未確認の命令コードやバイナリ値を断定的に載せる
- 画像内に外部ブランド名や別サイト名を入れる
- `PLC-FUN.com` など記事と無関係なサイト名・ロゴを入れる
- 英語記事用画像に日本語を混ぜる
- 日本語記事用画像に不要な英語を大量に混ぜる

#### GX Works3命令語画像のよい方向性
ADD/SUB、MOV、比較命令、カウンター、タイマーなどのGX Works3命令語画像では、以下の方向性を優先する。

- 1枚の画像で1テーマに絞る
- 命令の意味がひと目で分かる
- `source` / `operation` / `destination` の関係が分かる
- ラダー風の図は、細かい実装より「読み方」を伝えるために使う
- 命令ブロックは大きく、文字は少なめにする
- スマホでも読める文字サイズにする
- 色は白背景・青系中心にし、補助色として緑・黄・オレンジを少し使う
- 赤は注意・NG・危険のみに使う
- 先輩・後輩キャラクターを入れる場合は、必ず character template に合わせる
- キャラクターは図解補助にとどめ、命令ブロックやラダー図より目立たせない

#### 参考にしてよいが、そのまま採用しない例
以下のような画像は、構成の一部は参考にしてよいが、そのまま採用しない。

- ADD/SUBの命令概要、命令形式、ラダー例、動作例、まとめが整理された教育スライド風画像
- ラダー風表現や命令ブロック表現は参考にしてよい
- ただし、英語記事用に日本語が入っている場合はNG
- 外部ブランド名が入っている場合はNG
- キャラクターテンプレートに合わない人物が入っている場合はNG
- サイト全体の色味から外れている場合はNG
- 1枚に情報が多すぎる場合は、hero / overview / comparison / check-flow などに分割する

#### ADD/SUB画像で特に使ってよい表現
`gxw3-add-sub-instruction-basic` の画像では、以下は使ってよい。
...
- `D20 - K1 → D20`
- 簡略ラダー風の接点と命令ブロック
- `condition ON → calculate → write result` の流れ
- 公式画面ではない自作の命令ブロック図
...
### 5.6-a) 既存上位記事の画像再監査
- GSC上位記事の監査では、既存画像を「あるから維持」ではなく、技術的正確性・情報密度・モバイル可読性・キャラクター整合性で再評価する。
- 低品質と判断した画像は、記事構造を崩さず同じ役割の高品質画像へ差し替える。
- ただし技術図は、公式資料で確認できない端子・配線・接続関係を想像で描かない。
- 画像差し替えは本文の意味変更とは分離して管理し、PreviewでPC/スマホ双方を確認する。

### 5.7) 「あわせて読みたい」関連記事カードの画像ルール
- 記事末尾の「あわせて読みたい」「関連記事」カードは、画像付きカードを標準とする。
- 関連記事カードの画像は、対象記事の **OGP画像を第一候補** とする。
- OGP画像がカード表示に不向きな場合のみ、対象記事のhero画像を使用してよい。
- heroを使用する場合は、縮小表示でも記事テーマが判別できることを確認する。
- 新規記事では、OGP画像を関連記事カードにも流用できるよう、縮小時の視認性を意識して設計する。
- 関連記事カードの画像内文字が小さすぎる、情報量が多すぎる、主要要素がトリミングで欠ける場合は使用しない。
- 関連記事カード用に新しい別画像を必須とはしない。原則は OGP → hero の順で既存採用画像を再利用する。
- 画像がない既存記事は、当面テキストカードでもよい。無理に低品質画像を追加しない。
- 関連記事画像は遅延読み込み（`loading="lazy"`）を基本とし、レイアウト崩れ防止のため可能な限り `width` / `height` または `aspect-ratio` を指定する。
- PC・スマホの両方で、画像・記事タイトル・短い説明が読みやすいことをPreviewで確認する。
