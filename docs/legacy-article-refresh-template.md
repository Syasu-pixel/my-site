...
- ヒーロー画像とOGP画像を両方点検する。
- HeroとOGPの兼用は可能だが必須にしない。記事内表示と共有サムネイルで最適構図が異なる場合は、専用OGPを用意する。
- 同一画像で両用途を十分満たす場合は、不要な画像を増やさない。
- 人物を入れる場合は `assets/images/guide-characters/` の既存2人を基準にする。
- 画像フォルダは記事スラッグに合わせる。
- 画像は用途ごとに別ファイルにする。

### 6.2 OGP更新ルール
- 古い記事で共通OGPや汎用画像を使っている場合は、記事固有のHeroまたは専用OGPへの差し替えを検討する。
- HeroをそのままOGPへ流用する前に、OGP比率で主題が十分大きいか、文字や要点が潰れないか、記事内容が一目で分かるかを確認する。
- Heroとして適切でもOGPとして弱い場合は、Heroを無理に作り直さず専用OGPを分離する。
