...
## 右カラムの関連カテゴリ / Category links 運用ルール
- 新規記事HTMLでは、右カラムの「関連カテゴリ」または `Category links` カードは原則入れない。
- 右カラムは基本的に以下の2つに絞る。
  1. 見るポイント / Key points
  2. このサイトを応援する / Support this site
- カテゴリ導線は以下で担保する。
  - パンくず
  - Step 2でのカテゴリページ追加
  - `assets/data/search-index.json`
  - `sitemap.xml`
- 関連記事導線は、本文下部の「あわせて読みたい記事」または `related-grid` に集約する。
- コピー元テンプレートに「関連カテゴリ」または `Category links` が存在していても、新規記事では自動で残さない。
- 実在カテゴリページがある場合でも、右カラムへカテゴリカードを入れるのは原則避ける。
- 例外的に右カラムへカテゴリカードを入れてよいのは、複数カテゴリをまたぐまとめ記事・ハブ記事・カテゴリ選択そのものが読者の次行動になる記事だけ。
- 例外で入れる場合も、実在しないカテゴリURLを推測で作らない。
- `../index.html` やトップページへの退避リンクを、カテゴリ風の文言で右カラムに置かない。
- 日本語記事の「関連カテゴリ」カード、英語記事の `Category links` カードは同じ方針で扱う。
...
- メーカー公式資料を参照して本文・技術説明・安全表現・画像を作成または更新する場合は、docs/reference-notes/{slug}.md を必ず作成または更新する。
...
- 英語トップ `en/index.html` の `New articles` は7件固定。新規追加時は古い1件を押し出し、8件以上にしない
- 日本語トップ `index.html` の `新着記事` も7件固定。新規追加時は古い1件を押し出し、8件以上にしない
...
- [ ] マーカーペン風強調は、PE・FG・SG記事の `.mark` と同じ薄い青・文字下半分の見せ方を標準としている
