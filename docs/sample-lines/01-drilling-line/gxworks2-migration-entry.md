...
この構成では本体40DI / 40DOに16DIを追加し、合計64DI / 40DOとなる。現在要求の約58DI / 32DOを収容でき、既存FX5U版で使用している入力 `X0～X67` と出力 `Y0～Y34` を大きく崩さず移植しやすい。
...
現行要求 `約58DI / 32DO` を基準とし、既存 `physical-io-map.md` の信号名と機能を維持する。
...
- 入力増設：`FX2N-16EX` + `FX2N-8EX-ES/UL` 一次採用
...
## 10. 2026-09-19 第4回仕様反映

CV05とCY06追加により、GX Works2版の要求I/Oを約58DI / 32DOへ更新した。従来の56DI構成は不足するため、8点入力増設を追加して64DIを確保する。
