...
for(const path of pages)for(const width of [320,390,768,1440]){
 ...
 try{
  ...
  await page.locator('#dc-menu-toggle').click();assert(await page.locator('.dc-alternative').count()===(home?1:0),'Invented/missing translation');if(home)assert(await page.locator('.dc-alternative').getAttribute('href')===(en?'/':'/en/'),'Top translation target');await page.keyboard.press('Escape');
