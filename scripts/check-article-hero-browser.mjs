...
function compare(a,b){const x=PNG.sync.read(a),y=PNG.sync.read(b);if(x.width!==y.width||x.height!==y.height)throw Error('Image dimensions differ');let pixels=0;for(let i=0;i<x.data.length;i+=4)if(x.data[i]!==y.data[i]||x.data[i+1]!==y.data[i+1]||x.data[i+2]!==y.data[i+2]||x.data[i+3]!==y.data[i+3])pixels++;return pixels;}
...
try{for(const width of [320,390,768,1440]){
 ...
 for(const path of targets){let shots={};try{
  ...
  const topPixels=compare(shots.beforeTop,shots.afterTop),heroPixels=compare(shots.beforeHero,shots.afterHero),metricsExact=JSON.stringify(measurements.before)===JSON.stringify(measurements.after);
  if(topPixels||heroPixels||!metricsExact)throw Error('Hero display changed: '+JSON.stringify({topPixels,heroPixels,metricsExact}));
  ...
  results.push({path,variant:manifest.pageVariants[path],width,height:1000,topPixels,heroPixels,metricsExact,ctaCount:measurements.after.links.length,ctaTargets:measurements.after.links,hashes:Object.fromEntries(Object.entries(shots).map(([k,v])=>[k,hash(v)])),files});
 }catch(error){failures.push({path,width,error:error.message});for(const [part,data] of Object.entries(shots))await writeFile(resolve(evidence,path.replaceAll('/','__')+'--'+width+'--failure-'+part+'.png'),data);}
