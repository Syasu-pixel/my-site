...
const untouched=ledger.pages.length-config.targets.length-additional.length;
await writeFile(resolve(out,'publication-boundary-checks.json'),JSON.stringify({status:'passed',exactCandidateArticles:config.targets.length,additionalShellPages:additional.length,nonArticleUntouched:untouched,serviceNonShellBytesPreserved:true,reviewInjectionRejected:true,sourceOverwriteRejected:true,rejectionLeftOutputUnchanged:true},null,2));
console.log(JSON.stringify({publicationBoundary:'passed',articles:config.targets.length,additionalPages:additional.length,untouchedPages:untouched,reviewInjectionRejected:true,additionalReviewInjectionRejected:true,sourceOverwriteRejected:true}));
