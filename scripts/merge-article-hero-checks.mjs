...
for(let index=0;index<count;index++){
 ...
 if(part.index!==index||part.count!==count||JSON.stringify(part.targets)!==JSON.stringify(expected)||part.failures.length)throw Error('Incomplete/failed hero shard');
  for(const row of part.results){const key=row.path+'@'+row.width;if(coverage.has(key)||!expected.includes(row.path)||![320,390,768,1440].includes(row.width)||row.topPixels||row.heroPixels||!Number.isFinite(row.maxChannelDelta)||row.maxChannelDelta!==0||!row.metricsExact)throw Error('Invalid hero result');coverage.add(key);results.push(row);}
...
const summary={status:'passed',articles:manifest.targets.length,viewportPairs:results.length,comparedImagePairs:results.length*2,ctaClicks:actions.length,pixelDifferences:0,rawDifferentPixels:results.reduce((n,r)=>n+r.topRawPixels+r.heroRawPixels,0),maxChannelDelta:Math.max(...results.map(r=>r.maxChannelDelta)),metricsDifferences:0,cssExceptions:manifest.cssExceptions,variants:manifest.variants,results,actions,productionPublished:false};
