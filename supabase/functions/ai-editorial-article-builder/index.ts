...
const NETLIFY_SITE=Deno.env.get("AI_EDITORIAL_NETLIFY_SITE")??"frolicking-ganache-2a4a38";
...
const cls=["summary-card--fit","summary-card--notyet","summary-card--conclusion"];const sum=`<section aria-label="この記事の要点" class="top-summary" id="summary">${b.summary_cards.map((c:any,i:number)=>`<div class="summary-card ${cls[i]}"><h2>${esc(c.title)}</h2><ul>${c.items.map((v:any)=>`<li>${esc(v)}</li>`).join("")}</ul></div>`).join("")}</section>`;
h=replaceOnce(h,/<section aria-label="この記事の要点" class="top-summary" id="summary">[\s\S]*?<\/section>/i,sum,"summary");
...
h=replaceOnce(h,/<section aria-label="記事内メニュー" class="mini-toc-card">[\s\S]*?<\/section>/i,toc,"toc");
const sections=b.sections.map((s:any)=>`<section class="section-card" id="${esc(s.id)}"><h2>${esc(s.heading)}</h2>${sanitizeHtml(s.html)}</section>`).join("\n");
const talk=`<section class="section-card"><h2>先輩・後輩で確認</h2><div class="talk-thread"><div class="talk-row"><div class="talk-avatar"><img src="../assets/images/guide-characters/friendly_worker_with_helmet_and_smile.png" alt="先輩の案内役"></div><div class="talk-bubble"><p><span class="talk-name talk-name--senpai">先輩</span>${esc(b.conversation.senpai)}</p></div></div><div class="talk-row talk-row--right"><div class="talk-avatar"><img src="../assets/images/guide-characters/curious_worker_with_a_cheerful_expression.png" alt="新人の案内役"></div><div class="talk-bubble"><p><span class="talk-name talk-name--kouhai">後輩</span>${esc(b.conversation.kouhai)}</p></div></div></div></section>`;
