...
Deno.serve(async(req:Request)=>{
  ...
  const supplied=input.options&&typeof input.options==="object"&&!Array.isArray(input.options)?input.options as Record<string,unknown>:{};
  const options={...supplied,execution_model:"chatgpt-editor-in-chief",auto_process:false,queue_policy:"one-deliverable-one-queue",retry_policy:"same-queue",artifact_policy:"one-url-per-queue"};
  try{
    const rpc=await fetch(`${SUPABASE_URL}/rest/v1/rpc/ai_editorial_submit_command`,{
      method:"POST",
      headers:{Authorization:`Bearer ${token}`,apikey:ANON_KEY,"Content-Type":"application/json",Accept:"application/json"},
