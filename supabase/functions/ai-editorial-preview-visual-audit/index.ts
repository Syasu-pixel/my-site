...
Deno.serve(async req=>{
  ...
  try{
    ...
    const [statuses,checkRuns,arts,directOk]=await Promise.all([
      gh(`/commits/${sha}/status`).catch(()=>({statuses:[],state:"unknown"})),
      gh(`/commits/${sha}/check-runs`).catch(()=>({check_runs:[]})),
      gh(`/actions/artifacts?name=${encodeURIComponent(artifactName)}&per_page=20`).catch(()=>({artifacts:[]})),
      previewReachable(String(st.preview_url))
    ]);
    const statusList=Array.isArray(statuses?.statuses)?statuses.statuses:[];
    const netlifyStatus=statusList.find((x:any)=>String(x?.context||"").includes("netlify/")&&String(x?.context||"").includes("deploy-preview"));
    const checkList=Array.isArray(checkRuns?.check_runs)?checkRuns.check_runs:[];
    const netlifyCheck=checkList.find((x:any)=>/netlify|deploy preview|deploy-preview/i.test(String(x?.name||"")));
    const arr=Array.isArray(arts?.artifacts)?arts.artifacts.filter((a:any)=>!a.expired&&String(a?.workflow_run?.head_sha||"")===sha):[];
    const art=arr.sort((a:any,b:any)=>Date.parse(b.created_at)-Date.parse(a.created_at))[0];
    const captureReady=Boolean(art);
    const netlifyReady=netlifyStatus?.state==="success"||(netlifyCheck?.status==="completed"&&netlifyCheck?.conclusion==="success")||directOk;
    const readiness=netlifyReady?"netlify-success":captureReady?"capture-success":netlifyStatus?.state??netlifyCheck?.status??statuses?.state??"unknown";
    const verifiedBy=netlifyReady?"netlify-http-or-check+github-actions-playwright+gpt-vision":"github-actions-playwright+gpt-vision";
    const checks=Number(st.preview_probe_checks||0)+1;
    let next={...st,preview_probe_checks:checks,preview_last_status:readiness,preview_last_checked_at:new Date().toISOString(),preview_head_sha:sha,preview_capture_artifact_name:artifactName,preview_verified_by:verifiedBy};

    if(!netlifyReady&&!captureReady){await checkpoint(raw,id,next);return J({ok:true,status:"waiting-preview",preview_status:readiness,checks})}
    if(!art){
      next={...next,preview_capture_checks:Number(next.preview_capture_checks||0)+1,preview_capture_mode:"pull-request-auto"};
    ...
    await event(raw,id,"PC/スマホのPreview画面を自動取得しました。GPTが本文密度・図解・キャラクター・機器配置・レスポンシブ表示を監査しています。","BUILDING","info",{stage:"gpt-preview-audit-start",artifact_id:art.id,pr_number:pr,preview_readiness:readiness});
