...
function expectedTokenForAction(action: unknown): string {
  if (action === "delete") {
    return Deno.env.get("ARTIFACT_CLEANUP_TOKEN") || Deno.env.get("ARTIFACT_BROKER_TOKEN") || "";
  }
  if (action === "create_upload" || action === "prepare_download" || action === "import_firestorage") {
    return Deno.env.get("ARTIFACT_BROKER_TOKEN") || "";
  }
  return "";
}

function requireActionAuth(req: Request, action: unknown): boolean {
  const configured = expectedTokenForAction(action);
...
function validFirestorageId(value: unknown, length: number): value is string {
...
async function validateAndStore(supabase: ReturnType<typeof createClient>, fileName: string, expectedSha: string, bytes: Uint8Array) {
  ...
  if (!detectedMime) throw new Error("artifact signature rejected");
  if (!extensionMatches(fileName, detectedMime)) throw new Error("extension does not match content");
  const actualSha = await sha256Hex(bytes);
  if (!safeEqual(actualSha, expectedSha)) throw new Error("SHA-256 mismatch");
  const day = new Date().toISOString().slice(0, 10);
  const objectPath = `incoming/${day}/${crypto.randomUUID()}/${fileName}`;
  const { error: uploadError } = await supabase.storage.from(BUCKET).upload(objectPath, bytes, { contentType: detectedMime, upsert: false });
  if (uploadError) throw new Error("could not store artifact");
  const { data, error } = await supabase.storage.from(BUCKET).createSignedUrl(objectPath, DOWNLOAD_TTL_SECONDS, { download: false });
  if (error || !data?.signedUrl) throw new Error("could not create download URL");
  return { objectPath, downloadUrl: data.signedUrl, actualSha, detectedMime };
}
...
Deno.serve(async (req) => {
  ...
  if (!expectedTokenForAction(action)) return json(400, { error: "unknown action or broker secret missing" });
  ...
  if (action === "create_upload") {
    ...
    if (!fileName || !mimeType || !extensionMatches(fileName, mimeType)) return json(400, { error: "invalid file name or MIME type" });
    const day = new Date().toISOString().slice(0, 10);
    const objectPath = `incoming/${day}/${crypto.randomUUID()}/${fileName}`;
    ...
    return json(200, { object_path: objectPath, upload_url: data.signedUrl, expires_in_seconds: 7200, max_bytes: MAX_BYTES, mime_type: mimeType });
  }

  if (action === "import_firestorage") {
    ...
    try {
      ...
      return json(400, { error: error instanceof Error ? error.message : "firestorage import failed" });
    }
  }
  ...
  if (action === "prepare_download") {
    ...
    const expectedSha = typeof body.sha256 === "string" ? body.sha256.toLowerCase() : "";
    if (!validObjectPath(objectPath) || !/^[0-9a-f]{64}$/.test(expectedSha)) return json(400, { error: "invalid object path or SHA-256" });
    ...
    return json(200, { object_path: objectPath, download_url: data.signedUrl, expires_in_seconds: DOWNLOAD_TTL_SECONDS, sha256: actualSha, size_bytes: blob.size, mime_type: detectedMime });
  }
