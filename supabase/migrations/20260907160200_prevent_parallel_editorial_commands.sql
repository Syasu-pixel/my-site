...
begin
  ...
  -- Only one live editorial command may exist at a time. The advisory lock also
  -- prevents two near-simultaneous clicks from racing past the existence check.
  perform pg_advisory_xact_lock(hashtext('ai_editorial_single_active_command')::bigint);

  select c.id, c.status
    into v_active_id, v_active_status
  ...
  if found then
    raise exception 'active editorial command exists: % (%)', v_active_id, v_active_status
      using errcode = '55000',
            hint = 'Complete, automatically resume, or close the current command before creating another one.';
  end if;
  ...
  values (v_uid, v_instruction, v_count, coalesce(p_options, '{}'::jsonb) || jsonb_build_object(
